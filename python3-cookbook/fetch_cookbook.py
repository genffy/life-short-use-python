"""
copy from https://github.com/yidao620c/python3-cookbook/blob/master/notebook/fetch_cookbook.py
"""
...
IPYNB_PATH = os.path.join(os.path.dirname(__file__), "ipynb")
...
class Chapter:
    ...
    def fetch_list(self):
        ...
        *_, section_name = self.chapter_address.split("/")
        self.chapter_name = section_name.split(".")[0]
    ...
    def fetch_content(self, url):
        ...
        for tag in [x for x in soup.descendants if x.name]:
            if p_header.search(tag.name):
                ...
                cell["source"].append(
                    "{} {}\n".format(
                        "#" * (int(p_header.search(tag.name).group("level")) + 1),
                        tag.text,
                    )
                )
                cells.append(cell)
...
def fetch_all(sep=False):
    ...
    soup = BeautifulSoup(content, "html.parser")
    chapters = soup.find_all(
        "a", class_="reference internal", href=re.compile("chapters/p\d+")
    )
    for x in chapters[:15]:
        ch = Chapter(
            "https://python3-cookbook.readthedocs.io/zh_CN/latest/" + x["href"]
        )
        ch.fetch_list()
